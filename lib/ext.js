...
var middleware=function(req,res,next){
    if (req.url.indexOf('/ext/') === 0){
        ...
        if(files.indexOf(url_part)>=0){
            res.sendfile(path.join(__dirname,exports.version,url_part));
        }else{
...
walk(path.join(__dirname,exports.version),function(err,res){
    files = res;
});
